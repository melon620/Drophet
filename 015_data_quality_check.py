...
def main():
    ...
    # SMILES_1 (Primary Drug) is mandatory for model input.
    # Updated column names to match 014 output: SMILES_1
    smiles_col_1 = 'SMILES_1' if 'SMILES_1' in df.columns else 'Canonical_SMILES_1'
    smiles_col_2 = 'SMILES_2' if 'SMILES_2' in df.columns else 'Canonical_SMILES_2'

    df_cleaned = df.dropna(subset=[smiles_col_1]).copy()
    ...
    # Check Drug 1
    df_cleaned['is_valid_1'] = df_cleaned[smiles_col_1].apply(validate_smiles)

    # Check Drug 2 (if exists)
    if smiles_col_2 in df_cleaned.columns:
        df_cleaned['is_valid_2'] = df_cleaned[smiles_col_2].apply(
    ...
    target_cols = [col for col in df_cleaned.columns if str(col).startswith('Target_')]
